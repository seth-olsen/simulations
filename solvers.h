		  ...
		  PAR *p)
{
  ...
  get_ell_res_abpclean_join(res_0, f_xi, f_pi, f_al, f_be, f_ps, p, p->r, p->lastpt);
  dbl res = norm_inf(res_0);
  int ell_itn = 0;
  while (res > ell_tol) {
    VD jac(ldab*N_0, 0);
    set_jacCMabpslow(jac, f_xi, f_pi, f_al, f_be, f_ps, p, p->r, p->lastpt + 1, kl, ku, ldab);
    ...
    apply_up_abp_join(res_0, f_al, f_be, f_ps, p->r, p->lastpt + 1);
    get_ell_res_abpclean_join(res_0, f_xi, f_pi, f_al, f_be, f_ps, p->r, p->lastpt);
    res = norm_inf(res_0);

    if (outgoing_null_b(f_al, f_be, f_ps, p->r, p->lastpt) <= 0) {
      cout << "\nt = 0\nitn = " << ell_itn << "\nres = " << res << endl;
      return -(p->lastpt);
    }
    int k = lastpt;
    while (--k > 0) {
      if (outgoing_null(f_al, f_be, f_ps, p->r, k) <= 0) {
	cout << "\nt = 0\nitn = " << ell_itn << "\nres = " << res << endl;
	return -k;
      }
    }
    if (outgoing_null_f(f_al, f_be, f_ps, p->r, 0) <= 0) {
      cout << "\nt = 0\nitn = " << ell_itn << "\nres = " << res << endl;
      return -(p->lastpt + 1);
    }
		 ...
		 int N, int kl, int ku, int nrhs, int ldab, vector<int>& ipiv, int ldb)
{
  ...
  while (res > r[HYPTOL]) {
    ...
    while (res > r[ELLTOL]) {
      ...
      apply_up_abp_join(res_ell, f_al, f_be, f_ps, p, p->r, lastpt + 1);
      get_ell_res_abp_fast(res_ell, f_xi, f_pi, f_al, f_be, f_ps, p, p->r, lastpt);
      res = norm_inf(res_ell);
      if (++ell_itn > maxit) {
  ...
  while (res > r[HYPTOL]) {
    ...
    while (res > r[ELLTOL]) {
      ...
      info = LAPACKE_dgbsv(LAPACK_COL_MAJOR, N, kl, ku, nrhs,
			   &jac[0], ldab, &ipiv[0], &res_ell[0], ldb);
      if (info != 0) { cout << i*r[DTVAL] << "\nERROR: cannot solve elliptic equations\ninfo = " << info << endl; }
      apply_up_ab(res_ell, f_al, f_be, p, p->r, lastpt + 1);
      get_ell_res_ab_slow(res_ell, f_xi, f_pi, f_al, f_be, f_ps, p, p->r, lastpt);
      res = norm_inf(res_ell);
      if (++ell_itn > maxit) {
	cout << endl << i << " elliptic solver STUCK at t = " << i*r[DTVAL] << endl;
		     ...
		     int N, int kl, int ku, int nrhs, int ldab, vector<int>& ipiv, int ldb)
{
  ...
  while (res > r[HYPTOL]) {
	...
	res_hyp[kps + k] = fda_hyp_resPs(old_ps, f_ps, cn_xi, cn_pi, cn_al, cn_be, cn_ps, r, k);
      ...
      res_hyp[kps + lastpt] = fdaR_hyp_resPs(f_ps, p, p->r, lastpt);
